...
public class Gameplay extends JPanel implements KeyListener, ActionListener{
	...
	public void paint(Graphics g) {
		...
		if(totalBrick <= 0) {
			...
			g.drawString("Apuyer Sur Entrer pour Recommencer",230,350);
		...
		if (ballposeY > 570) {
			...
			g.setFont(new Font("serif",Font.BOLD,20));
			g.drawString("Apuyer Sur Entrer pour Recommencer",130,350);
			...
			a:for(int i = 0; i < map.map.length;i++) {
				for(int j = 0; j< map.map[0].length;j++) {
					if(map.map[i][j] > 0) {
						...
						int bricY = i*map.brickHeight+50;
						int brickWidth = map.brickWidth;
						int brickHeight = map.brickHeight ;
						
						Rectangle rect = new Rectangle(brickX,bricY,brickWidth,brickHeight);
